        ...
        if (hp <= 0){
            if (totalDmgReceived != 0) {
                if (totalPowerUpReceived < std::abs(totalDmgReceived)){
                    min_hp = std::abs(totalPowerUpReceived + totalDmgReceived) + 1; //dmg is negative, then +
...
public:
    ...
    Room* getRoom(int rowIdx, int colIdx){
        return rooms[rowIdx][colIdx];
    }
};

class AbstractDungeonSolver {
public:
    virtual int solve() = 0;
    virtual ~AbstractDungeonSolver() {}
};

class RecursiveDungeonSolver : public AbstractDungeonSolver {
    Dungeon* dungeon;
    Knight* knight;

public:
    RecursiveDungeonSolver(Dungeon* dungeon, Knight* knight)
        : dungeon(dungeon), knight(knight) {}
...
public:
    State(int hp, Room* r, size_t i, size_t j)
        : requiredHp(hp), room(r), row(i), col(j) {}
    ...
    bool operator>(const State& other) const {
        return requiredHp > other.requiredHp;
    }
};


class DijkstraDungeonSolver : public AbstractDungeonSolver {
    Dungeon* dungeon;

public:
    DijkstraDungeonSolver(Dungeon* dungeon) : dungeon(dungeon) {}

    int solve() override {
        size_t rows = dungeon->getRows();
        size_t cols = dungeon->getCols();

        std::vector<std::vector<int>> minHP(rows, std::vector<int>(cols, INF));
        std::priority_queue<State, std::vector<State>, std::greater<>> pq;

        Room* end = dungeon->getPrincessRoom();
        int endPoints = end->getPoints();
        int startHP = std::max(1, 1 - endPoints);
        pq.emplace(startHP, end, rows - 1, cols - 1);
        minHP[rows - 1][cols - 1] = startHP;

        while (!pq.empty()) {
            State current = pq.top(); pq.pop();
            int hp = current.getRequiredHp();
            size_t r = current.getRow();
            size_t c = current.getCol();

            for (auto [dr, dc] : std::vector<std::pair<int, int>>{{-1, 0}, {0, -1}}) {
                size_t nr = r + dr, nc = c + dc;
                if (nr < rows && nc < cols) {
                    Room* neighbor = dungeon->getRoom(nr, nc);
                    int need = std::max(1, hp - neighbor->getPoints());
                    if (need < minHP[nr][nc]) {
                        minHP[nr][nc] = need;
                        pq.emplace(need, neighbor, nr, nc);
                    }
                }
            }
        }
